...
import { DataGridProps, GridLocaleText, GridSlotsComponent } from '@mui/x-data-grid';
import { UncapitalizeObjectKeys } from '@mui/x-data-grid/internals';

export class GridTableAttribute {
  private static readonly SIZE_OPTIONS: number[] = [10, 20, 30, 40, 50];
  private static readonly LOCALE_TEXT: Partial<GridLocaleText> = {
  ...
  public static Default: Pick<
    DataGridProps,
    'rowSelection' | 'paginationMode' | 'sx' | 'localeText' | 'pageSizeOptions'
  > = {
    localeText: this.LOCALE_TEXT,
    pageSizeOptions: this.SIZE_OPTIONS,
    rowSelection: true,
    paginationMode: 'server',
    sx: { position: 'sticky' },
  };

  public static Slot: Pick<UncapitalizeObjectKeys<Partial<GridSlotsComponent>>, 'loadingOverlay'> = {
