...
export const ProjectPageSaleRecordUpdateDialogAction: FunctionComponent<{
  row: ProjectRecordRow;
  body: ProjectRecordUpdateBody;
  onClose: () => void;
}> = ({ row, body, onClose }) => {
  const disabled = buttonHook.useDisabledByObject(row, body);
  const callback = projectRecordHook.useUpdateCallback(ProjectRecordType.Sale, row.id, body);
