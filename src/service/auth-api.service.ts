...
export class AuthApiService extends Api {
  ...
  async updatePassword(body: AuthUpdatePasswordBody): Promise<ApiResponse<void>> {
    body.currentPassword = HashService.getInstance().toHex(body.currentPassword);
    body.newPassword = HashService.getInstance().toHex(body.newPassword);
    body.confirmPassword = HashService.getInstance().toHex(body.confirmPassword);

    return this.valueFrom<void>(this.patch('/auth', body));
  }

  async signin(body: AuthSignInBody): Promise<ApiResponse<AuthResponse>> {
    body.password = HashService.getInstance().toHex(body.password);
